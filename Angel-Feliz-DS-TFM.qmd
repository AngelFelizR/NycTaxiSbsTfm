...
- **Fechas y tiempos**: `request_datetime`, `on_scene_datetime`, `pickup_datetime` \newline y `dropoff_datetime`.
...
- **Datos del Censo de EE. UU.**: Variables demográficas y socioeconómicas a nivel de sector censal para los distritos de interés. Se utilizaron tablas de la Encuesta sobre la Comunidad Estadounidense (ACS) 2022 y el Censo Decenal 2020.
- **Datos de OpenStreetMap**: Enriquecemos los datos con características geoespaciales derivadas de OpenStreetMap (OSM) para proporcionar una visión más profunda de cada zona.
...
5.  **Procesamiento en Memoria con `data.table`**: Una vez extraídos los datos desde DuckDB o recuperados desde la caché, las operaciones de manipulación, agregación y transformación de grandes volúmenes de datos en memoria se resolvieron mediante `data.table` [@r_data_table]. Esta librería resultó fundamental para el motor de simulación Monte Carlo y el procesamiento de los resultados intermedios, ya que permite realizar filtrados, uniones y agrupaciones de manera altamente eficiente, reduciendo significativamente los tiempos de cómputo durante las iteraciones del modelo y garantizando la escalabilidad del flujo de trabajo.
...
Después de simular estas condiciones, podemos utilizar los resultados de Monte Carlo para inferir la distribución del salario medio diario por hora. Los ingresos promedio por hora se sitúan entre **\$54.97** y **\$55.21**, con un 95% de confianza, y una media de $55.09 dólares por hora. Estos resultados son los mostrados en @fig-baseline-hist mediante el uso de `ggplot2` y `ggtext` [@r_ggplot2; @ggtext2026]
...
El preprocesamiento de datos fue una fase extensa que combinó transformaciones estándares, ingeniería de variables temporales y espaciales, y la integración de fuentes de datos externas para enriquecer el conjunto de entrenamiento. Todo este proceso se encapsuló en una receta (`recipe`) de `tidymodels` para garantizar su reproducibilidad [@kuhn2022tidy; @r_tidymodels].

### Variables Espaciales

Dada la alta cardinalidad de las variables categóricas de origen (`PULocationID`) y destino (`DOLocationID`) (cerca de 200 zonas TLC), estas fueron sustituidas por variables derivadas de información geoespacial y socioeconómica asociada a cada zona.

* **Información geoespacial (OpenStreetMap)**: Se descargaron datos de OpenStreetMap utilizando los paquetes `osmdata` y `sf` para Manhattan, Brooklyn y Queens [@osmdata2017]. Para cada zona TLC se calcularon variables derivadas de diferentes tipos de elementos geográficos: densidad de puntos (por ejemplo, `amenity` y `shop`), longitud de líneas (por ejemplo, `highway`) y proporción de área cubierta por polígonos (por ejemplo, `landuse`). Todas las métricas se normalizaron por el área de la zona para hacer comparables zonas de distintos tamaños. Tras el proceso de validación y selección, se conservaron **22 variables geoespaciales**.

* **Información socioeconómica (Censo de EE. UU.)**: Se utilizaron `tidycensus` y datos de la **American Community Survey (ACS) 2022** y del **Censo Decenal 2020** para obtener variables a nivel de sector censal [@tidycensus2026]. Estas variables se agregaron a las zonas TLC mediante una unión espacial, calculando una media ponderada según la proporción del área de cada sector censal dentro de la zona. Dado el gran número de variables disponibles (aproximadamente **160,000**), primero se realizó una selección automática basada en técnicas de procesamieOpenStreetMapnto de lenguaje natural (NLP). En particular, se agruparon variables con descripciones similares utilizando la **distancia de Jaccard** y detección de comunidades mediante **modularidad**, y posteriormente se identificaron las palabras más representativas de cada grupo mediante **TF-IDF**, lo que permitió seleccionar las categorías temáticas más relevantes y reducir el conjunto a **1,433 variables**. Finalmente, se conservaron únicamente aquellas que mostraban una mayor diferencia entre las medianas de los viajes aceptados y rechazados, obteniendo un conjunto final de **41 variables socioeconómicas**.


### Variables Temporales

- **Ciclos Diarios y Semanales**: En lugar de usar la hora y el día de la semana de forma lineal, se utilizaron transformaciones seno y coseno (`step_harmonic`) para representar la naturaleza cíclica del tiempo  [@embed2026]. Esto permite que el modelo aprenda patrones como "las 23:00 es similar a la 01:00", algo que los valores numéricos lineales no capturan bien.
- **Festivos**: Se generaron variables indicadoras y de "días hasta" para los principales festivos de EE. UU. (`step_holiday`) [@timeDate2025].
- **Franjas Horarias**: Se crearon variables binarias para AM/PM y se extrajeron componentes como el mes, semana, día del año, etc [@lubridate2021].
...
- **Balanceo de Clases**: Dado el desbalance inherente entre las clases generadas por el etiquetado con *lookahead* (aproximadamente un 25% de viajes etiquetados como "aceptables" frente al 75% restante), se aplicó una técnica de *downsampling* sobre la clase mayoritaria directamente en la receta de preprocesamiento. Esta estrategia, implementada mediante la función `step_downsample()` del paquete `themis` [@themis2025], permite que el modelo XGBoost aprenda patrones más representativos de los viajes realmente rentables, reduciendo el sesgo hacia la clase negativa y mejorando la calibración de las probabilidades predichas.

## Modelo de Clasificación (XGBoost) para Viajes de Alto Valor

Para predecir si un viaje en oferta pertenece al **25% más rentable** de las alternativas disponibles en los próximos 15 minutos, se entrenó un modelo de clasificación **XGBoost** [@r_xgboost]. El proceso de entrenamiento fue supervisado gracias a un etiquetado basado en *lookahead*.
...
3. **Ventana temporal**: La hora de solicitud debe estar entre 3 segundos (tiempo necesario para rechazar el viaje actual) y 15 minutos después de la solicitud original.
...
#| fig-cap: "Distribución del salario medio por hora para las 65.856 combinaciones de inicio; el 10% superior supera los $70 por hora"
#| fig-num: true
#| fig-height: 4
